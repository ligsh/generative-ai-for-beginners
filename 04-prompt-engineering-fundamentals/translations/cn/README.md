...
3. **本地环境变量** - _现在，需要完成SETUP步骤，以做好准备。
...
## Illustrated Guide 这一节整个在中文版本中缺失

如果你现在深入研究之前知道本课（this lession）覆盖的主要内容，check out这个illustrated guide，本guide将让你对本课所覆盖主题有个大致感觉。本课的路线图带着你从理解核心概念和挑战，到使用相关的提示工程技术以及最佳实践来应对这些挑战。注意，本guid中的“高级技术”部分将在下一课中讲述。
这里有个图片，就是所属的guide。
...
- **教育者**可能会要求人工智能 _为目标受众和主题生成教学计划_。 AI 可以按照指定的格式构建个性化计划。
...
在本章中，我们将**提示工程**定义为 _设计和优化_ 文本输入（提示) 的过程，以便为给定的应用程序目标和模型提供一致且高质量的响应（完成) 。 我们可以将其视为一个两步过程：
...
LLM 将提示视为 _标记序列(sequence of token)_，其中不同的模型（或模型的不同版本）可以以不同的方式对同一提示进行标记(tokenize翻译为“分词”更合适？）。 由于 LLM 是根据标记（tokens, 而不是原始文本）进行训练的，因此提示标记化的方式(将“提示”进行“分词”的方式)对生成的响应的质量有直接影响。

要直观地了解标记化(tokenization)的工作原理，请尝试使用如下所示的 [OpenAI Tokenizer](https://platform.openai.com/tokenizer?WT.mc_id=academic-105485-koreyst) 等工具。 复制您的提示 - 并查看如何将其转换为标记，注意空白字符和标点符号的处理方式。 请注意，此例子显示的是较旧的 LLM (GPT-3) - 因此使用较新的模型尝试此操作可能会产生不同的结果。
...
想了解幻觉是如何运作的吗？ 想象一个提示，指示人工智能为不存在的主题(即要确保在训练数据集中没有相关的信息)生成内容。 例如 - 我尝试了这个提示：
...
让我们从基础提示开始：发送到模型的文本输入，没有其他上下文。 这是一个例子 - 当我们将美国国歌的前几个单词发送到 OpenAI [Completion API](https://platform.openai.com/docs/api-reference/completions?WT.mc_id=academic-105485-koreyst) 时，它会立即用后面的几行 _完成_ 了响应，说明了基本的预测行为。
...
现在让我们为基础提示添加上下文和说明。 [Chat Completion API](https://learn.microsoft.com/azure/ai-services/openai/how-to/chatgpt?WT.mc_id=academic-105485-koreyst) 让我们以 _messages_ 集合的方式构建复杂的提示，这些消息包含：

- 反映 _用户_ 输入和 _助理_ 响应的输入/输出对。
- _系统消息_ 设置助理行为或个性化。

该请求现在采用以下形式，其中 _标记化(tokenization)_ 有效地从上下文和对话中捕获相关信息。 现在，更改系统上下文对补全(completions)质量的影响，与用户输入产生的影响一样重要。
...
在上面的示例中，用户提示是一个简单的文本查询，可以解释为信息请求。 通过 _指令(instruction)_ 提示，我们可以使用该文本更详细地指定任务，从而为人工智能提供更好的指导。 这是一个例子：
...
## 主要内容Primary Content(这是一种提示类型)

在上面的示例中，提示仍然相当开放，允许 LLMs 决定其预训练数据集的哪一部分是相关的。 使用 _主要内容_ 设计模式，输入文本分为两部分：
...
主要内容片段可以通过多种方式来驱动(产生)更有效的指令：

- **示例(的方式)** - 不要用明确的指令告诉模型要做什么，而是给它提供要做什么的示例，并让它进行推断。
- **暗示(的方式)** - 遵循带有“暗示”的说明，引导完成，引导模型做出更相关的响应。
- **模板(的方式)** - 这些是带有占位符（变量）提示的可重复“配方”，可以使用特定用例的数据进行自定义。

### 示例方式

这种方法中，您使用primary content来“向模型提供”希望给定指令能够照样输出的一些示例，并让它推断所需输出的模式。 根据提供的示例数量，我们可以有零样本提示、单样本提示、少样本提示等。
...
使用主要内容的另一种技术是提供 _暗示_ 而不是示例。 在这种情况下，我们通过使用一个片段（这个片段能够反映所希望的响应的格式）来让模型朝正确的方向进行推理。 然后，模型“接受提示”，继续按照这种思路进行。
...
如果我们将提示构建视为具有指令（任务) 和目标（主要内容），那么 _次要内容_ 像附加上下文，我们可以用它来**以某种方式影响输出**。 它可以是调整参数、格式化指令、主题分类法等，可以帮助模型定制其响应以适应所需的用户目标或期望。
